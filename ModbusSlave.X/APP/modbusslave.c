...
static uint16_t iInputRegisters[10] = {2, 15, 65};

static uint16_t iHoldingRegisters[5] = {10, 20, 30, 40, 50};
...
void modbusParsePacket()
{
    if(crcCheck() == true)
    {
        ...
        {
            buildModbusPacket();            //function call
            sendModbusPacket();
        }
    }
}
...
*/
void buildModbusPacket()        //function definition
{
    ...
    giTxIndex = 0;
    gcTxBuf[giTxIndex++] = gcRx_buf[MB_SID_INDEX];;
...
void readHoldingRegister()
{
    ...
    for (int i = 0; i < (iByteCount / 2); i++)//3
    {
...
/********************************************************************************/
/*
 * GETCRC16 IS A VARIABLE USED TO STORE VALUE OF CRC CALCULATED BY USING GCRXBUF
